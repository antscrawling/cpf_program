...
def login_page() -> bool:
    ...
    if forgot_btn:
        ...
        if username in users:
            ...
            sa_button = st.button("Verify Secret Answer")
            canc_btn = st.button("Cancel")
            if canc_btn:
                # Cancel password reset and return to login mode
                st.session_state["register_mode"] = False
                st.session_state["logged_in"] = False
                #st.session_state.pop("username", None)
                st.rerun()
            ...
            # Initialize a verification flag in session state
               ...
               # st.session_state["secret_verified"] = False
                ...
                if check_password(secret_answer_input.lower(), users[username]["secret_answer"]):
                    ...
                    new_password1 = st.text_input("Enter new password", type="password", key="reset_pass1")
                    new_password2 = st.text_input("Confirm new password", type="password", key="reset_pass2")
                    
                    submit_btn = st.button("Submit New Password")
                    cancel_btn = st.button("Cancel")
