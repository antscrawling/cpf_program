...
DATABASE_NAME = os.path.join(  # noqa: PTH118
...
def main():
    with CPFAccount() as cpf:
        ...
        if not all([startdate, enddate, birthdate]):
            msg = "Missing required date values in the configuration file. Please check 'startdate', 'enddate', and 'birthdate'."
            raise ValueError(
                msg,
            )
        ...
        print(
            f"{'Month and Year':<15}{'Age':<5}{'OA Balance':<15}{'SA Balance':<15}{'MA Balance':<15}{'RA Balance':<15}{'Loan Amount':<12}{'Excess Cash':<12}{'CPF Payout':<12}"
        )
        print("-" * 150)
        ...
        if is_initial:
            ...
            for account, new_balance in zip(
                ...
                cpf.record_inflow(
                    account=account,
                    amount=new_balance,
                    message=f"Initial Balance of {account}",
                )
            is_initial = False
        ...
        with create_connection() as conn:
            ...
            for date_key, date_info in tqdm(
                ...
                if year == 1 and cpf._loan_balance > 0:
                    cpf.record_outflow(
                        account="oa",
                        amount=loan_paymenty1,
                        message=f"Loan payment from OA Account at year 1 age {cpf.age}",
                    )
                    cpf.record_outflow(
                        account="loan",
                        amount=loan_paymenty1,
                    ...
                    cpf.record_outflow(
                        account="oa",
                        amount=loan_paymenty1,
                        message=f"Loan payment from OA Account at year 2 age {cpf.age}",
                    )
                    cpf.record_outflow(
                        account="loan",
                        amount=loan_paymenty1,
                    ...
                    cpf.record_outflow(
                        account="oa",
                        amount=loan_paymenty3,
                        message=f"Loan payment from OA Account at year 3 age {cpf.age}",
                    )
                    cpf.record_outflow(
                        account="loan",
                        amount=loan_paymenty3,
                    ...
                    if cpf._loan_balance > 0:
                        ...
                        cpf.record_outflow(
                            account="oa",
                            amount=loan_payment,
                            message=f"Loan payment from OA Account at year 4, age {cpf.age}",
                        )
                        cpf.record_outflow(
                            account="loan",
                            amount=loan_payment,
                ...
                # Step 15 at the age of 55, SA Balance is closed and transferred to RA.  OA Balance is also transferred to RA.
                    ...
                    for account in ["oa", "ma", "ra"]:
                        ...
                        cpf.record_inflow(
                            account=account,
                            amount=allocation_amount.__round__(2),
                            message=f"Allocation for {account} at age {cpf.age}",
                        )
                ...
                if cpf.current_date.month == 12:
                    ...
                    for account in ["oa", "sa", "ma", "ra"]:
                        cpf.message = (
                            f"Applying interest for {account} at age {cpf.age}"
                        )
                        account_balance = getattr(cpf, f"_{account}_balance", 0.0)
                        if account_balance > 0:
                            if account == "oa":
                                oa_interest = round(
                                    cpf.calculate_interest_on_cpf(
                                        account=account, amount=account_balance
                                    ),
                                    2,
                                )
                            elif account == "sa":
                                ...
                                ma_interest = cpf.calculate_interest_on_cpf(
                                    account=account, amount=account_balance
                                ).__round__(2)
                            elif account == "ra":
                                ra_interest = cpf.calculate_interest_on_cpf(
                                    account=account, amount=account_balance
                                ).__round__(2)
                    ...
                    (
                        ...
                        ma_extra_interest,
                        ra_extra_interest,
                    ) = cpf.calculate_extra_interest()
                    cpf.record_inflow(
                        account="oa",
                        amount=oa_interest,
                        message=f"Interest for {account} at age {cpf.age}",
                    )
                    cpf.record_inflow(
                        account="sa",
                        amount=sa_interest,
                    ...
                    cpf.record_inflow(
                        account="oa",
                        amount=oa_extra_interest.__round__(2),
                        message=f"Extra Interest for {account} at age {cpf.age}",
                    )
                    cpf.record_inflow(
                        account="sa",
                        amount=sa_extra_interest.__round__(2),
                        message=f"Extra Interest for {account} at age {cpf.age}",
                    )
                    cpf.record_inflow(
                        account="ma",
                        amount=ma_extra_interest.__round__(2),
                        message=f"Extra Interest for {account} at age {cpf.age}",
                    )
                    cpf.record_inflow(
                        account="ra",
                        amount=ra_extra_interest.__round__(2),
                        message=f"Extra Interest for {account} at age {cpf.age}",
                    )
                    ...
                    if isinstance(cpf.payout, (int, float)):
                        ...
                        if cpf._ra_balance > 0:
                            cpf.record_outflow(
                                account="ra",
                                amount=cpf.payout,
                                message=f"CPF payout at age {cpf.age}",
                            )
                            cpf.record_inflow(
                                account="excess",
                                amount=cpf.payout,
                ...
                if cpf._ra_balance == 0.0 and cpf.age > 55:
                    print(
                        f"Stopping simulation at age {cpf.age} as RA balance is zero."
                    )
                    break
                ...
                if is_display_special_july:
                    ...
                    print(
                        f"{display_date_key:<15}{cpf.age:<4}"
                    ...
                    cpf.record_inflow(
                        account="oa",
                        amount=display_oa_bal,
                        message=f"transfer_cpf_age={cpf.age}",
                    )
                    cpf.record_inflow(
                        account="sa",
                        amount=display_sa_bal,
                        message=f"transfer_cpf_age={cpf.age}",
                    )
                    cpf.record_inflow(
                        account="loan",
                        amount=display_loan_bal,
                        message=f"transfer_cpf_age={cpf.age}",
                    )
                    cpf.record_inflow(
                        account="ra",
                        amount=display_ra_bal,
                        message=f"transfer_cpf_age={cpf.age}",
                    )
                    cpf.record_inflow(
                        account="excess",
                        amount=display_excess_bal,
                        message=f"transfer_cpf_age={cpf.age}",
                    )
                ...
                if cpf.age == 55:
                    cpf.message = "Age 55 - Special case for CPF payout"
