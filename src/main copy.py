...
def main():
    ...
    st.session_state.setdefault("logged_in", False)  # Ensure logged_in state exists
    mylogin = login_page()
    if st.session_state["logged_in"] and mylogin:
...
def main_page():
    ...
    with col6:
        if st.button("🛑 EXIT"):
            ...
            st.stop()  # This will stop Streamlit execution and show a "stopped" message
            # Alternatively, to forcefully exit (not recommended for most cases):
...
def login_page() -> bool:
    ...
    if quit_btn:
        st.write("Exiting the application...")
        # This will stop Streamlit execution and show a "stopped" message
        # Alternatively, to forcefully exit (not recommended for most cases):
        # import os; os._exit(0)
        st.session_state["logged_in"] = False
        st.rerun()
